...
if ($_SESSION['role'] == 'etudiant') {
    ...
    $sql = "SELECT DISTINCT note.*, etudiant.nom AS etudiant_nom, etudiant.prenom AS etudiant_prenom, module.nom_module 
    FROM note
    INNER JOIN etudiant ON etudiant.id = note.etudiant_id
    INNER JOIN module ON module.id = note.module_id
    WHERE note.etudiant_id = $etudiantId";
       
       $result = $conn->query($sql);
       $_SESSION['note_sql_query']=$sql;
...
$sql = "SELECT DISTINCT note.*, etudiant.nom AS etudiant_nom, etudiant.prenom AS etudiant_prenom, module.nom_module 
        ...
        INNER JOIN module_formateur ON module_formateur.module_id = note.module_id AND module_formateur.formateur_id = $formateurId
        INNER JOIN module ON module.id = note.module_id
...
if ($result->num_rows > 0) {
    ...
    if (isset($_POST['submit'])) {
        $search = $_POST['search'];
    // Retrieve all courses within the admin's center
    $sql = "SELECT note.*, etudiant.nom AS etudiant_nom, etudiant.prenom AS etudiant_prenom, module.nom_module
            FROM note
            INNER JOIN etudiant ON etudiant.id = note.etudiant_id
            INNER JOIN module ON module.id = note.module_id
    ...
    $result = mysqli_query($conn, $sql);
    $_SESSION['note_sql_query']=$sql;
    // Fetch the notes for the courses within the admin's center
    $etudiants = [];
    if ($result->num_rows > 0) {
...
if (isset($_GET['success']) && $_GET['success'] == "ajouter") {
    echo '<div class="alert alert-success" role="alert">
              Note a été Ajouter success
          </div>';

    echo '<script>
              if (typeof window.history.replaceState === "function") {
                  window.history.replaceState({}, "", "note.php");
              }
          </script>';
}
?>
...
<a href="note-pdf.php" class="btn btn-outline-primary me-2"><i class="fas fa-download"></i> Download</a>
<?php  if ($_SESSION['role'] == 'formateur') {?><a href="add-note-etudiant.php" class="btn btn-primary"><i class="fas fa-plus"></i></a><?php }?>
</div>
</div>
</div>
...
<td><?php echo $etudiant['etudiant_prenom']; ?></td>
<td><?php echo $etudiant['nom_module']; ?></td>
<td><?php echo $etudiant['valeur']; ?></td>
